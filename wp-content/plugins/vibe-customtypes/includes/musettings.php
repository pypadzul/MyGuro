	...
	function lms_general_settings(){
		...
		switch($_GET['sub']){
			...
			default:
				...
				array(
			            'label' => __('Remove expired Courses from directory', 'vibe-customtypes'),
			            'name' => 'remove_expired_course',
			            'desc' => __('Auto remove expired courses from course directory for user', 'vibe-customtypes'),
			            'type' => 'checkbox',
					),
				...
				array(
						'label' => __('Enable React Quizzes','vibe-customtypes'),
						'name' => 'react_quizzes',
						'type' => 'checkbox',
						'desc' => __('Quizzes will be rendered with reactjs (faster, optimized, lesser load on server)','vibe-customtypes')
					),
				array(
						'label' => __('Enable Question Advanced Stats','vibe-customtypes'),
						'name' => 'question_advanced_stats',
						'type' => 'checkbox',
						'desc' => __('Only available for react quizzes','vibe-customtypes')
					),
				array(
			...
			default:
				...
				$sync_settings = apply_filters('wplms_sync_settings',array(
					...
					array(
						...
						'label'=>__('Delete Old Question Comments','vibe-customtypes'),
						'description'=>__('Deletes unnecessary comments from your site helps optmizing comments table','vibe-customtypes'),
					),
					array(
						'id'=>'vibe_payouts',
						'label'=>__('Migrate payouts','vibe-customtypes'),
						'description'=>__('Migrate payouts to new structure for graphs','vibe-customtypes'),
					),
					array(
						'id'=>'calculate_question_correct_percentage',
						'label'=>__('Calculate question correct percentages','vibe-customtypes'),
						'description'=>__('Calculate question\'s correct percentage to show in quiz stats','vibe-customtypes'),
					),
				));
...
//MANISH's CODE --- 
		...
		if(!empty($currencies)){
			echo '<p><strong>'.__('Set Threshold Commissions','vibe-customtypes').':</strong></p>';
		...
		echo '<input type="submit" class="button" name="threshold_btn" value="'.__('Save','vibe-customtypes').'">';
		...
		if($count) {
			...
			echo '<input type="submit" class="button show_request_payouts" name="show_payouts" value="'.__('Show','vibe-customtypes').'"></form></div>';
			...
			if(!empty($new_instructor_data) && is_array($new_instructor_data)){
				...
				if(!empty($instructor_c_commissions)){
					...
					<table>
					  ...
					  <thead>
					    <tr>
					      <th scope="col">'.__('Instructor Name','vibe-customtypes').'</th>
					      <th scope="col">'.__('Commission','vibe-customtypes').' ('.get_woocommerce_currency_symbol($current_currency).')</th>
					      <th scope="col">'.__('Payouts','vibe-customtypes').' ('.get_woocommerce_currency_symbol($current_currency).')</th>
					      <th scope="col">'.__('Commission to pay','vibe-customtypes').' ('.get_woocommerce_currency_symbol($current_currency).')</th>
					      <th scope="col">'.__('PAYPAL EMAIL','vibe-customtypes').'</th>
					      <th scope="col">'.__('Select','vibe-customtypes').'</th>
					      <th scope="col">'.__('Pay via PayPal','vibe-customtypes').'</th>
					    </tr>
					  </thead>
					  <tbody>';
					//loop 
					...
					foreach ($instructors as $instructor) {
				        ...
				        echo '<tr>
					      <td scope="row" data-label="'.__('Instructor Name','vibe-customtypes').'">'. $instructor->user_nicename.'</td>

					      <td data-label="'.__('Commission','vibe-customtypes').'">'.(isset($instructor_c_commissions[$instructor->ID])?$instructor_c_commissions[$instructor->ID]:0).'</td>

					      <td data-label="'.__('Payouts','vibe-customtypes').'">'. (isset($paid_payouts[$instructor->ID][$current_currency])?$paid_payouts[$instructor->ID][$current_currency]:0).'</td>

					      <td data-label="'.__('Commission to pay','vibe-customtypes').'"><input type="text" id="'.$instructor->ID.'_amount" name="instructor['.$instructor->ID.'][commission]" class="text" value="'.(isset($instructor_c_commissions[$instructor->ID])?($instpayout>=0)?$instpayout:0:0).'" /></td>

					      <td data-label="'.__('PAYPAL EMAIL','vibe-customtypes').'"><input type="text" id="'.$instructor->ID.'_email" name="instructor['.$instructor->ID.'][email]"  value="' . $instructor_email . '" /></td>

					      <td data-label="'.__('Select','vibe-customtypes').'"><input type="checkbox" name="instructor['.$instructor->ID.'][set]" class="checkbox" value="1" /></td>

					      <td data-label="'.__('Pay via PayPal','vibe-customtypes').'"><a id="'.$instructor->ID.'_payment" class="button">'.__('Pay via PayPal','vibe-customtypes').'</a></td>
					    </tr>
                            ...
                            jQuery(document).ready(function($){
                                $("#'.$instructor->ID.'_payment").click(function(){
                                    ...
                                    $.ajax({
				                      	...
				                      	cache: false,
				                      	success: function (html) {
                                    		$(\'<form name="_xclick" action="https://www.paypal.com/in/cgi-bin/webscr" method="post" target="_blank"><input type="hidden" name="cmd" value="_xclick"><input type="hidden" name="business" value="\'+email+\'"><input type="hidden" name="currency_code" value="'.$current_currency.'"><input type="hidden" name="item_name" value="'.__('Instructor Commission','vibe-customtypes').'"><input type="hidden" name="amount" value="\'+amount+\'"></form>\').appendTo($this).submit();
				                      	}
				                    });
	...
	function get_addons(){
		return apply_filters('wplms_lms_addons',array(
			...
			'wplms-quiz-certificates' =>array(
					'label'=> __('WPLMS Quiz Certificates','vibe-customtypes'),
					'sub'=> __('Enable Quiz only certificates','vibe-customtypes'),
					'icon'=> '<span class="dashicons dashicons-portfolio"></span>',
					'requires'=> '',
...
class wplms_miscellaneous_settings{
	...
	function sync_resync(){
		...
		switch($_POST['id']){
			...
			case 'vibe_payouts':
			global $wpdb;
				$data = $wpdb->get_results("SELECT p.ID as payout_id FROM {$wpdb->posts} as p WHERE p.post_type = 'payments'");
				$payouts = array();
				if(!empty($data)){
			...
			global $wpdb;
				$data = $wpdb->get_results("SELECT c.comment_ID as comment_id,p.ID as id FROM {$wpdb->comments} AS c LEFT JOIN {$wpdb->posts} as p ON p.ID = c.comment_post_ID WHERE p.post_type = 'question'");
				$payouts = array();
				if(!empty($data)){
	...
	function end_calculate_question_correct_percentage_sync(){
		...
		echo __('Quiz correct percentage stats calculated','vibe-customtypes');
		die();
	}
	...
	function end_delete_old_question_comments_sync(){
		...
		echo __('Deleting old question comments complete !','vibe-customtypes');
		die();
	}
	...
	function end_vibe_payouts_sync(){
		if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'sync_resync')){
	         _e('Payouts structure upgraded.','vibe-customtypes');
	        die();
		}
		echo __('Unit - attachments sync complete !','vibe-customtypes');
		die();
